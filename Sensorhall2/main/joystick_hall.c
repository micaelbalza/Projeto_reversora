...
static esp_err_t jh_init_nvs(void)
{
    esp_err_t err = nvs_flash_init();
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ESP_LOGW(TAG, "NVS precisa erase (no free pages / new version).");
...
esp_err_t joystick_hall_init(const joystick_hall_config_t *cfg)
{
    ...
    if (jh_load_from_nvs() == ESP_OK) {
        ESP_LOGI(TAG, "Calibracao carregada do NVS.");
    } else {
        s_cal_loaded = false;
        ESP_LOGW(TAG, "Sem calibracao no NVS (precisa calibrar).");
    }
